...
void debuggingSuite() {
	...
	std::vector<MachineAction> lastNCycles;
	...
	char inputChar = '0';
	while (inputChar != 'q') {
		std::cout << std::endl << std::setfill('-') << std::setw(20) << '-' << std::endl << std::endl;

		// Printing the PPU registers.
		std::cout << "PPU Registers: " << std::endl;
			std::cout << "    PPUCTRL: " << displayHex(ppu.registers.PPUCTRL, 2)
			<< "\n    PPUADDR: " << displayHex(ppu.registers.PPUADDR, 4)
			<< "\n    PPUDATA: " << displayHex(ppu.registers.PPUDATA, 2) << std::endl << std::endl;

		msg = " --- What to perform --- \n q: quit\n s: see last (n) instructions\n     S: see last (n) PPU cycles\n u: undo the last instruction\n     U: undo (n) instructions\n r: dump registers\n d: dump memory from (x) to (y)\n e: execute next instruction\n     E: execute (n) instructions\n     A: execute until (address) or (i) cycles\n o: toggle output; currently (" + btos(outputResults, "ON", "OFF") + ")\n c: Clear stack of executed cycles.\n Option: ";
		...
		case('s'):
			...
			std::cout << std::endl;
			for (CycleAction& instruction : lastNInstructions) {
				if (instruction.instructionExecuted) {
					instruction.executedInstruction->print();
					std::cout << std::endl;
				}
				else {
			...
			break;
		case('S'):
			msg = "How many machine cycles would you like to see? Give a positive integer value: ";
			numOfInstr = abs(input.getUserInt(msg));
			if (numOfInstr <= 0) {
				break;
			}
			lastNCycles = nes.getMachineActions(numOfInstr);
			std::cout << std::endl;
			for (MachineAction& action : lastNCycles) {
				action.print();
				std::cout << std::endl;
			}
			break;
